...
function formatMetaTitle(title) {
  const suffix = " | ames.consulting";
  const maxLength = 70;
  ...
  return `${title.slice(0, maxLength - suffix.length - 3).trimEnd()}...${suffix}`;
}
...
function generatePages() {
  ...
  for (const post of data.posts) {
    ...
    const html = template
      .replace(/{{META_TITLE}}/g, formatMetaTitle(post.title))
