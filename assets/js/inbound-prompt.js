...
const variants = [
  {
    match: /\/(photography|portraits|family|corporate-cup|girls-on-the-run|giron|sweat-heart|bike-fitting)/,
...
function currentVariant() {
  return variants.find((variant) => variant.match.test(location.pathname)) || {
    type: "Something else",
...
function scrollProgress() {
  ...
  return available <= 0 ? 1 : scrollY / available;
}

function initInboundPrompt() {
  if (location.pathname.includes("/contact") || document.querySelector("[data-inbound-prompt]")) return;
  ...
  const maybeOpen = () => {
    if (hasEnoughTime && hasEnoughScroll && !wasRecentlyDismissed()) open({ automatic: true });
  };
  const stopTimer = () => {
    if (!timer) return;
    clearTimeout(timer);
