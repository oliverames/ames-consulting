...
- Google Analytics 4 (measurement ID `G-YF4LQ85VRE`) is the one third-party script. `assets/js/google-tag.js` holds the config call and appends the `www.googletagmanager.com` loader only on the production hostnames, so no inline script or `'unsafe-inline'` is needed and non-production hosts (local, CI, `pages.dev` previews) send no hits. The CSP admits only the analytics collection hosts, not Google's advertising endpoints. `scripts/google-tag.mjs` is the source of truth for the ID, production hosts, and CSP host lists.
