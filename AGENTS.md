...
- **google-tag.js** — Classic (non-module) script injected right after the charset meta on every page by `apply-shared-ui.mjs`. Holds the `gtag('config', 'G-YF4LQ85VRE')` call and appends Google's async loader itself, but only when `location.hostname` is `ames.consulting` or `www.ames.consulting`, so dev servers, Playwright, CI Lighthouse, and `pages.dev` previews send nothing and never download the 175 KB loader. Kept external so CSP stays `script-src 'self'` plus hosts. Measurement ID, production hosts, and CSP host lists live in `scripts/google-tag.mjs`.
